...
require_once ( dirname(__FILE__).'/iCalUtilityFunctions.class.php' );
/*********************************************************************************/
 ...
 */
  function parse( $unparsedtext=FALSE ) {
    ...
    //Remove  espços em branco no final do arquivo ics
    for($i = $lix; $i >= 0; $i--)
    {
        if(strtoupper(trim($rows[$i])) == 'END:VCALENDAR')
            break;
         else
           unset($rows[$i]);
    }
 ...
 */
  function _makeUid() {
    ...
    /*
     * Modificado por Cristiano Correa Schmidt
     */
        //$this->uid['value']  = $date.'-'.$unique.'@'.$this->getConfig( 'unique_id' );
    $this->uid['value']  = $this->getConfig( 'unique_id' );
  }
/**
...
// echo $this->objName.'<br />'.var_export( $this->unparsed, TRUE )."<br />\n"; // test ###
    ...
    foreach( $this->unparsed as $line ) {
      ...
      if( $newProp ) {
        ...
        if(( 1 < strlen( $proprows[$lastix] ))         &&
           (  ' ' == substr( $line, 0, 1 ))) {
          $proprows[$lastix] = substr( $proprows[$lastix], 0, strlen( $proprows[$lastix] ) - 1 );
          $line = substr( $line, 1 );
        }
        $proprows[$lastix] .= $line;
      }
    ...
    foreach( $proprows as $line ) {
      ...
      for( $cix=0; $cix < $clen; ++$cix ) {
        ...
        if('"' == $line[$cix] && $controlCN)  $controlCN = false;
        if('"'   == $line[$cix] && strtoupper( substr( $line, $cix - 3, 3 )) == 'CN=' )  $controlCN = true;
