...
function draw_message(info_msg, ID){
	...
	option_hide_more.onclick = function(){
		...
		option_mark_as.onclick = function(){
			...
			if (thi.value != 'hide'){
				thi.value = 'hide';
				option_mark_as_options.src= '../expressoMail1_2/templates/default/images/pressed.png';
	...
	option_hide_more.innerHTML = get_lang('Show details');
		...
		option_mark_as_options.onmouseover = function(){
			option_mark_as_options.src= '../expressoMail1_2/templates/default/images/over.png';
	...
	space2.innerHTML = '&nbsp;|&nbsp;';
		...
		option_more.onclick = function(){
			...
			if (option_more_options.value != 'hide'){
				...
				option_more_options.src= '../expressoMail1_2/templates/default/images/pressed.png';
		...
		option_more_options.onmouseover = function(){
			...
			option_more_options.src= '../expressoMail1_2/templates/default/images/over.png';
		};
	...
	var div_other_options = document.createElement("DIV");
		...
		var report_error = '<span onmouseover="this.className=\'reply_options_active\'" onmouseout="this.className=\'reply_options\'" class="reply_options" onclick=reports_window("'+currentTab+'");>'+get_lang("Report error")+'</span> | '; 
	...
	if (parserImport){
		$.ajax({
			...
			success: function(data){
				...
				case 5:
					$('.type_images').append('<img class="loader" src="templates/default/images/ajax-loader.gif" align="top" style="margin-left: 5px; cursor: pointer; display: inline">');
					$.ajax({
						url: "controller.php?action="+import_url+'&from_ajax=true&selected=true',
						success: function(msg){		
							$('.type_images').append('<img src="../prototype/modules/mail/img/flagDone.png" align="top" style="margin: 3px 0 0 5px; cursor: pointer; display: inline">').parent().find('.loader').remove();
					...
					break;		
				case 4:
					$('.type_images').append('<img src="../prototype/modules/mail/img/flagDone.png" align="top" style="margin: 3px 0 0 5px; cursor: pointer; ">');
...
function build_quota(data){
	...
	if(!q_limit){
		div.innerHTML = '<span><font size="2" style="color:red"><strong>'+get_lang("Without Quota")+'</strong></font></span>';
	}
	else{
		if(div)
			div.innerHTML = '';
		...
		td11.align="center";
		td11.setAttribute("noWrap","true");
		td11.innerHTML += '<div id="quota_progressbar" class="boxHeaderText" style="display: block; float: left; height: 13px; width: 121px;"></div>&nbsp;<span class="boxHeaderText" style="line-height : 0px;">' + value+"% ("+borkb(q_used*1024)+"/"+borkb(q_limit*1024)+")</span>";
