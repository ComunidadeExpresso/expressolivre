...
cRichTextEditor.prototype.execPosInstance = function(inst) {
	...
	editor.document.on('keydown', function(event)
	{
		...
		save_link.unbind("click").click(function(){
			openTab.toPreserve[id] = true;save_msg(id);
		});
	...
	{
		CKEDITOR.instances[inst].document.on('keydown', function(event){
...
cRichTextEditor.prototype.dataReady = function(id,reply)
{
	...
	if (this.plain[id]){
		...
		CKEDITOR.instances['body_'+id].on('dataReady',function(e){
			if (reply === 'forward' ){	
				setTimeout(function(){
						RichTextEditor.blur(id);
...
cRichTextEditor.prototype.setInitData = function (id,data,reply,recursion, callback){
	var content = $("#content_id_"+id);
...
cRichTextEditor.prototype.keydown = function (id,rec){
    if (rec === undefined) rec = 1;
    rec++;
    if( CKEDITOR.instances['body_'+ id] === undefined ) return;    
    var element = CKEDITOR.instances['body_'+ id];  
    
    if(element.document){
        element.document.on('keydown',function(){
            $("#content_id_"+currentTab+" .save").button("enable");
        });
    } else {
        if (rec <= 20)
                setTimeout(function(){RichTextEditor.keydown(id,rec)},500);
    }
}
//Build the Object
