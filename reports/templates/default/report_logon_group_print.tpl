   ...
   <div align="center">
			...
			<tr>
			  <td colspan="2" align="left">
			  	<div align="right" style="float:right">
					<form name="back" method="post" action="./index.php?menuaction=reports.uireports_logon.report_logon_group">
		      ...
		      <td width="9%" align="left">				  
				  <div align="right" style="float:left">
					<form action="./index.php?menuaction=reports.uireports_logon.report_logon_print_pdf" method="post" name="pdf" target="_blank">
					  ...
					  <input type='hidden' name='nacesso' value='{nacesso}'>									  			
				    </form>		      
			      </div>
		  </tr>
        </table>
		<table border="0" width="90%">
			...
			<tr>
		      ...
		      <td width="60%" align="left">{lang_total_users}&nbsp;<strong>{cont_user}</strong>			  </td>
	...
	<div align="center">
		<table border="0" width="90%">
			...
			<tr bgcolor="{th_bg}">
				...
				<td width="10%">{lang_Creation Date}</td>
				<td width="14%"><div align="center">{lang_Last access}</div></td>
			    <td width="8%"><div align="center">{lang_Days without login}</div></td>
			    <td width="8%">{lang_status}</td>
			</tr>
			{rows}
		</table>
	</div>
...
<!-- BEGIN row -->
	<tr bgcolor="{tr_color}">
		<form method="post" name="{formname}" action="./index.php?menuaction=reports.uireports_logon.show_access" >
		<td>{row_loginid}</td>
		<td>
			<div onClick="{formsubmit}">			
				<a href="#">{row_cn}</a>
				<input name="account_id" type="hidden" value="{row_idnumber}">
				<input name="nome_usuario" type="hidden" value="{row_cn}">
				<input type='hidden' name='setor' value='{sector_name}'>
				<input type='hidden' name='organizacao' value='{organizacao}'>
				<input type='hidden' name='setordn' value='{sector_namedn}'>
				<input type='hidden' name='organizacaodn' value='{organizacaodn}'>
				<input type='hidden' name='page' value='{page}'>
				<input type='hidden' name='gpage' value='{gpage}'>
				<input type='hidden' name='nacesso' value='{nacesso}'>				
				<input type='hidden' name='Psectornamedncompleto' value='{subtitulo1}'>
			</div>
		</td>
		</form>
		<td>{row_mail}</td>
		<td><div align="center">{row_timestamp}</div></td>
		<td><div align="center">{row_li_date}</div></td>
		<td><div align="center">{row_li_dias}</div></td>
		<td>{row_status}</td>
	</tr>
<!-- END row -->
<!-- BEGIN row_empty -->
