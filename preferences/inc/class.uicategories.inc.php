<?php
	...
	* option) any later version.                                               *
	...
	class uicategories
	{
		...
		{
            $edata_count = count($edata);
			for ($j=0;$j<$edata_count;++$j)
			{
		...
		function index()
		{
			...
			$this->bo->query = get_var('query',array('GET','POST'));
                        $this->bo->order = get_var('order',array('GET','POST'));
                        $this->bo->start = get_var('start',array('GET','POST'));
                        $this->bo->sort  = get_var('sort',array('GET','POST'));
                        $this->query = get_var('query',array('GET','POST'));
                        $this->order = get_var('order',array('GET','POST'));
                        $this->start = get_var('start',array('GET','POST'));
                        $this->sort  = get_var('sort',array('GET','POST'));
                        $GLOBALS['query'] = get_var('query',array('GET','POST'));
...
// -------------------------- end header declaration --------------------------------------
			...
			for ($i=0;$i<$cats_count;++$i)
			{
				...
				if ($cats[$i]['owner'] == $GLOBALS['phpgw']->accounts->id2name($this->account) && $cats[$i]['app_name'] != 'phpgw')
				{
