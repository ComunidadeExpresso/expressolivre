...
class Request {
	...
	$this->data = $this->getConfig($config, 'data', NULL, file_get_contents("php://input"));
        ...
        // get HTTP request dataDecode into array
	$this->dataDecoded  = $this->__decodeData( $this->data );
    ...
    protected function getResourceClassDetails($className) {
     ...
     /**
     * Convert the string data into a array data
     * @return array
     * @codeCoverageIgnore
     */
	...
	$arrayData = array();
	if($data){
	    $arrayIndex = explode('&', urldecode($data));
	    foreach($arrayIndex as $key => $value){
		list($i,$val) = explode ('=', $value);
		$arrayData[$i] = $val;
	    }
	}
	return $arrayData;
        ...
        if ($uriMatches) {
            ...
            if (!$resource['loaded']) { // autoload
                if (!class_exists($resource['class'])) {
                    throw new Exception('Unable to load resource' . $resource['class']);
                }
