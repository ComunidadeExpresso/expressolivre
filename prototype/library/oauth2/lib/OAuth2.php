<?php
require_once ROOTPATH . '/library/oauth2/lib/OAuth2ServerException.php';
require_once ROOTPATH . '/library/oauth2/lib/OAuth2AuthenticateException.php';
require_once ROOTPATH . '/library/oauth2/lib/OAuth2RedirectException.php';
...
class OAuth2 {
	...
	const DEFAULT_AUTH_CODE_LIFETIME = 3600;
	 ...
	 * @var string
	 */
	const CONFIG_ACCESS_LIFETIME = 3600; // The lifetime of access token in seconds.
	const CONFIG_REFRESH_LIFETIME = 3600; // The lifetime of refresh token in seconds.
	const CONFIG_AUTH_LIFETIME = 3600; // The lifetime of auth code in seconds.
	...
	public function verifyAccessToken($token_param, $scope = NULL) {
		...
		session_write_close(); //Fecha session anterior
		session_id($token['refresh_token']); //Define o id da session o mesmo do resfresh token
		@session_start();
		
		/* TODO - Verificar melhor solução para verificação de tratamento para sessions já startadas
		if(!isset($_SESSION))
		    session_start();
		*/
		return $token;
	...
	public function grantAccessToken(array $inputData = NULL, array $authHeaders = NULL) {
		...
		switch ($input["grant_type"]) {
			...
			case self::GRANT_TYPE_REFRESH_TOKEN:
				...
				session_write_close(); //Fecha session anterior
				session_id($stored['refresh_token']); //Define o id da session o mesmo do resfresh token
				session_start();
		...
		//Gerando Session Para o refresh_token
		session_write_close(); //Fecha session anterior
		session_start();
		session_id($token['refresh_token']); //Define o id da session o mesmo do resfresh token
		session_cache_expire(self::CONFIG_REFRESH_LIFETIME); //Define o tempo da session o mesmo do Refresh token	
		
		if(isset($input["username"]) && isset($input["password"]))
		{
		    $_SESSION['wallet']['user']['uid'] =     $input["username"];
		    $_SESSION['wallet']['user']['password']       =  $input["password"];
		    $_SESSION['wallet']['user']['uidNumber']       =  $user_id;
		}
		
		
		
		//session_write_close(); //Fecha nova session
...
//	}
	protected function createAccessToken($client_id, $user_id, $scope = NULL) {
		...
		$this->storage->setAccessToken($token["access_token"], $client_id, $user_id, time() + $this->getVariable(self::CONFIG_ACCESS_LIFETIME), $scope , $token['refresh_token']);
