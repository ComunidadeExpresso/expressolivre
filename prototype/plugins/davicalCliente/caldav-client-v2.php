...
require_once ROOTPATH.'/plugins/davicalCliente/XMLDocument.php';
...
class CalDAVClient {
  ...
  * @var string
  */
  public $user_agent = 'DAViCalClient ExpressoLivre';
...
//        echo "\nTags array............................................................\n";  print_r( $this->xmltags );
  ...
  */
  ...
  /**
  * Send an MOVE request to the server
  * Exp: (Move collection) $cal->DoMOVERequest( colloection1/ , collection2/ );
  * @param string $method origem
  * @param string $destination destination
  *
  * @return string The content of the response from the server
  */
  function DoMOVERequest( $origem, $destination) {
    $this->requestMethod = 'MOVE';
    $this->headers[] = 'Destination: '.$this->base_url.$destination;
    return $this->DoRequest($this->base_url.urlencode($origem));
  }
  ...
  */
  function DoPUTRequest( $url, $icalendar, $etag = null ) {
    ...
    var_dump( $this->httpResponseHeaders);
  ...
  */
  function FindCalendarHome( $recursed=false ) {
    if ( !isset($this->principal_url) ) {
      $this->FindPrincipal($this->principal_url);
    }
    if ( $recursed ) {
...
//          printf("Looking at: %s[%s]\n", $href, $v['tag'] );
          switch( $v['tag'] ) {
            ...
            case 'DAV::displayname':
              $calendar->displayname = isset($v['value']) ? $v['value'] : '';
              break;
          }
        }
        $calendars[] = $calendar;
