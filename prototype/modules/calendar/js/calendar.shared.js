function show_modal_shared(calendarId){
	...
	UI.dialogs.sharedCalendar = jQuery('#calendarShared').append('<div title="Compatilhamento de Agenda" class="shared-calendar active"> <div>').find('.shared-calendar.active').html(html).dialog({
	    resizable: false, 
	    modal:true, 
	    autoOpen: false,
	    width:620, 
	...
	if(event.keyCode == '13' || typeof(event.keyCode) == 'undefined') {						
	    ...
	    if (!result) {
		UI.dialogs.sharedCalendar.find('ul.search-result-list').append('<li><label class="empty">Nenhum resultado encontrado.</label></li>');
	    }
...
function show_modal_search_shared(){
    ...
    if (!UI.dialogs.sharedCalendar) {
	UI.dialogs.sharedCalendar = jQuery('#calendarShared').append('<div title="Pesquisar Agendas" class="shared-calendar active"> <div>').find('.shared-calendar.active').html(html).dialog({
	    resizable: false, 
	    modal:true, 
	    autoOpen: false,
	    width:620, 
