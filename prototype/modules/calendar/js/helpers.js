...
function printNow(){
	if($("#calendar").fullCalendar('getView').name == "agendaWeek" || $("#calendar").fullCalendar('getView').name == "basicWeek" || $("#calendar").fullCalendar('getView').name == "year")
		alert('A tela de impressão será melhor visualizada com a preferência "Paisagem" do seu browser selecionada.');
...
function printEvents(){
	...
	print.find('.fc-button-content').html('Imprimir');
...
function add_tab_configure_calendar(calendar, type) 
{
	...
	if(type == 0){
		...
		var tab_title = "Configurações de Grupos";
	}
...
function remove_calendar(type){
	...
	if(!!parseInt(type))
		var title = 'Todas as tarefas deste grupo serão removidas. Deseja prosseguir com a operação?';
	else
		var title = 'Todos os eventos desta agenda serão removidos. Deseja prosseguir com a operação?';
    $.Zebra_Dialog(title, {
	'type':     'question',
	'overlay_opacity': '0.5',
	'buttons':  ['Sim', 'Não'],
...
function refresh_calendars(type){
	...
	meu_container.find(".button.new-calendar").button({
        ...
        var typeCalendar = !!parseInt($(this).attr('class').match(/[0-9]+/g)) ? 
            {type: 'new-group', title: 'Novo Grupo', typeValue: 1, prompt: 'Nome do grupo'} : 
            {type: 'new-calendar', title: 'Nova Agenda', typeValue: 0, prompt: 'Nome da agenda'}
...
function show_modal_import_export(tab, calendarId, typeView){
	...
	if (!UI.dialogs.importCalendar) {
	    ...
	    .append('<div title="Importar e Exportar "' + (typeView == 0 ? 'Eventos' : 'Tarefas') + '" class="import-export import-export-win active"> <div>')
