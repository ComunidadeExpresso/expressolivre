...
{	
	...
	public final function getGroupsJmessenger()
	{
		...
		if( $this->db )
		{
			$query = "SELECT * FROM phpgw_config WHERE config_app = 'phpgwapi' AND config_name = 'groups_jmessenger_jabberit';";
	...
	public final function setAddGroupsJmessenger($pData)
	{
		...
		if( $this->db )
		{
			...
			if( count($result) == 0 )
				$query = "INSERT INTO phpgw_config( config_app, config_name, config_value ) VALUES('phpgwapi', 'groups_jmessenger_jabberit', '".$pData."');";
			else
				$query = "UPDATE phpgw_config SET config_value = '".$pData."' WHERE config_app = 'phpgwapi' AND config_name = 'groups_jmessenger_jabberit';";
