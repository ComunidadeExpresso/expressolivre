(function()
{
		...
		function addContact()
		{
			...
			{
				var jidFrom		= Base64.decode( loadscript.getUserCurrent().jid );
				...
				var _paramNewUser =
				{
					...
					'jidFrom'			: jidFrom,					
					'jidTo'				: jidTo,
					'selectBoxOptions'	: _groups					
				};
				
				var winNewUser =
				{ 
						id_window	 : "new_user_jabberit",
						width		 : 320,
						height		 : 165,
						top		 	 : 100,
						left		 : 350,
						...
						title		 : 'Expresso Messenger - ' + i18n.NEW_USER,
						closeAction  : "remove",
						content		 : Xtools.parse( Xtools.xml('new_user'), 'newUser.xsl', _paramNewUser)
				};

				_winBuild( winNewUser );
				
				// Select Editable
				loadscript.setSelectEditable(document.getElementById('name_group_new_user_jabberit'), 5, 99);
				...
				// Add Buttons
				document.getElementById('buttons_newuser').innerHTML = Xtools.parse(Xtools.xml('buttons_main'), 'buttons.xsl', _pButtons);

			}
			else
			{	
				var jidFrom		= Base64.decode( loadscript.getUserCurrent().jid );
				var jidTo		= getElement('user_jid_jabberIM').value;
				var name		= getElement('user_name_jabberIM').value;
				var group		= getElement('user_group_jabberIM').value;
				
				_winBuild('add_user_info','remove');
				
				if( jidFrom != jidTo )
				{
					TrophyIM.setAutorization( jidTo, jidFrom, 'subscribe' );
		...
		function addNewUser()
		{
			var name 	= getElement('name_new_user_jabberit').value;
			var group	= getElement('name_group_new_user_jabberit').value;
			var jidFrom	= getElement('jidFrom_new_user_jabberit').value;
			var jidTo	= getElement('jidTo_new_user_jabberit').value;
			...
			if( jidFrom != jidTo )
			{
				TrophyIM.rosterObj.removeContact( jidTo );
				
				// Remove Element
				loadscript.removeElement( getElement('itenContact_' + jidTo ) );
				loadscript.removeElement( getElement('span_show_itenContact_' + jidTo ) );
				loadscript.removeElement( getElement('itenContactNotification_' + jidTo ) );

				TrophyIM.addContact( jidTo, name, group );
				
				TrophyIM.setAutorization( jidTo, jidFrom, 'subscribe' );
				
				_winBuild("new_user_jabberit","remove");
			}
		...
		function search()
		{
			...
			if( _input.value.substring((_input.value.length-1),_input.value.length) === "*" )
				_input.value = _input.value.substring(0, ( _input.value.length - 1 ));

			if( _input.value.substring(0 ,1) === "*" )
				_input.value = _input.value.substring( 1, _input.value.length );
			...
			conn.go('p.cc.getListContacts',
					...
					{	
						...
						var _newUser = _div.firstChild;
						
						while ( _newUser )
						{
							if( _newUser.getAttribute('photo') === '1' )
							{
								var jid		= _newUser.getAttribute('jid'); 
								var ou	 	= _newUser.getAttribute('ou');
								
								var _img_path  = path_jabberit + 'inc/WebService.php?' + Date.parse( new Date );
									_img_path += '&photo_session=' + jid + '&ou=' + ou;
								
								_newUser.style.backgroundImage = 'url(' + _img_path + ')';
							}
							
							loadscript.configEvents(_newUser, 'onclick', showContact);
		...
		function showContact(Element)
		{
			...
			if( element.getAttribute('value') )
			{
				var infoUser = 
				{
					'email'				: element.getAttribute('value').substring( 0, element.getAttribute('value').indexOf(';')),
			...
			else if( element.parentNode.getAttribute('value') )
			{
				var infoUser = 
				{
					'email'				: element.parentNode.getAttribute('value').substring( 0, element.parentNode.getAttribute('value').indexOf(';')),
					'jid'				: element.parentNode.getAttribute('jid'),
					'lang_group'		: "Grupo",
					'lang_name_contact'	: "Contato",					
			...
			// Select Editable
			loadscript.setSelectEditable(document.getElementById('user_group_jabberIM'),35, 155);
			
			// Load Photo;
			var _img = null;
			
			if( element.style.backgroundImage )
				_img = element.cloneNode( false );
							
			if( element.parentNode.style.backgroundImage )
				_img = element.parentNode.cloneNode( false );
		...
		loadAddUser.prototype.newUser	= addNewUser;
