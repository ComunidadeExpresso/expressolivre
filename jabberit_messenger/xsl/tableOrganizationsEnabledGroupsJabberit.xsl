		...
		<table id="tableOrganizationsEnabledGroupsJabberit" cellspacing="2" style="width:100%">
			...
			<xsl:for-each select="return/ou	">
				<tr id="{.}" style="width:40%" class="row_off">
					<td class="row_on"><xsl:value-of select="." /></td>
					<td class="row_on" style="width:30% !important"><a href="javascript:constructScript.removeOrgGroupsLocked('{.}');"><xsl:value-of select="$lang2"/></a></td>
				</tr>
			</xsl:for-each>
		</table>
	</xsl:template>
