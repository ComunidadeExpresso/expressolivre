...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="preferences_jabberit">
		<fieldset style="margin:3px; padding:5px;border:1px solid #cecece;">
			...
			<fieldset style="height:60px;margin-bottom:6px;padding:5px; border:1px solid #cecece;">
				<legend><xsl:value-of select="$lang8"/></legend>
				<br/>
				<label><xsl:value-of select="$lang9"/> .: </label>
				<select id="openWindowJabberitPopUp">
					<option value="true"><xsl:value-of select="$langYes"/></option>
					<option value="false"><xsl:value-of select="$langNo"/></option>
				</select>
			</fieldset>
