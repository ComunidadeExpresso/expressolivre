<?php
	...
	* And if the register link must be placed                *
	...
	if ($_GET['cd']=='300' && $GLOBALS['phpgw_info']['server']['certificado']==1)
	{
		...
		$_SESSION['contador'] = 0;
		$valor_contador = 0;
		$link_alterna_login = '<img src="phpgwapi/templates/news/images/warning.gif"/><a href="login.php">' . lang('Access without Digital Certificate') . '</a>';
		$tmpl->set_var('show','none');
		$tmpl->set_var('action','<div id="action"><img style="border:0px;margin:31px 0px 58px 0px;" src="phpgwapi/templates/news/images/acao.gif" /></div>');
...
*/
	...
	else
	{
		if($GLOBALS['phpgw_info']['server']['certificado']==1)
		{
			...
			$link_alterna_login = '<img src="phpgwapi/templates/news/images/lock1_icon.gif"/><a title="' . lang('Link to use digital certificate') . '" href="login.php?cd=300">' . lang('Logon with my digital certificate') . '</a>';
		}
		$tmpl->set_var('lang_username',lang('username'));
		$tmpl->set_var('action','');
		$cod_applet = '';
	...
	// Load News from NewsAdmin module - BEGIN
	define('PHPGW_TEMPLATE_DIR','./phpgwapi/templates/'.$GLOBALS['phpgw_info']['login_template_set']);
	$news = CreateObject('news_admin.uinews');
	$public_news = $news->bo->get_all_public_news();
	rsort($public_news);	
	$newslist = array();
	foreach($public_news as $i => $news){
		$newslist[] = array_map('utf8_encode', $news);
	}
	$tmpl->set_var('public_news',json_encode($newslist));
	$tmpl->set_var('lang_previous',lang('Previous'));
	$tmpl->set_var('lang_next',lang('Next'));
	$tmpl->set_var('lang_pause',lang('Pause'));
	$tmpl->set_var('lang_resume',lang('Resume'));
	
	// Load News from NewsAdmin module - END
