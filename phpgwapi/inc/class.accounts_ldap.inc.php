	...
	class accounts_
	{
		...
		function read_repository()
		{
			...
			else
			{
				...
				$sri = @ldap_search($this->ds, $this->user_context, ("(&(uidnumber=" . (int)$this->account_id.")(|(phpgwaccounttype=u)(phpgwaccounttype=s)))"), $justthese);
			}
			if(!$sri)
				return null;
		...
		function delete($accountid = '')
		{
			...
			if(!$allValues[0]['dn'])
			{
				$sri = ldap_search($this->ds, $this->user_context, "(&(uid=" . (string)$account_lid.")(|(phpgwaccounttype=u)(phpgwaccounttype=s)))");
				if($sri)
				{
					$allValues = ldap_get_entries($this->ds, $sri);
		...
		function name2id($name,$which='account_lid')
		{
			...
			$sri = ldap_search($this->ds, $this->user_context, '(&('.$to_ldap[$which].'=' . (string)$name . ')(|(phpgwaccounttype=u)(phpgwaccounttype=s)))', array("uidnumber"));
			$allValues = ldap_get_entries($this->ds, $sri);
		...
		function id2name($account_id,$which='account_lid')
		{
			...
			$sri = @ldap_search($this->ds, $this->user_context, '(&(uidnumber=' . (int)$account_id . ')(|(phpgwaccounttype=u)(phpgwaccounttype=s)))', $justthese);
			if(!$sri)
				return False;
			$allValues = ldap_get_entries($this->ds, $sri);
		...
		function get_type($account_id)
		{
			$allValues = array();
			/* jakjr: using justthese with ldap_search */
			$sri = @ldap_search($this->ds, $this->user_context, '(&(uidnumber=' . (int)$account_id . ')(|(phpgwaccounttype=u)(phpgwaccounttype=s)))', array("phpgwaccounttype"));
			if(!$sri)
				return False;
			$allValues = ldap_get_entries($this->ds, $sri);
		...
		function id2fullname($account_id)
                {
			$allValues = array();
			$sri = @ldap_search($this->ds, $this->user_context, '(&(uidnumber=' . (int)$account_id . ')(|(phpgwaccounttype=u)(phpgwaccounttype=s)))');
			if(!$sri)
				return False;
			$allValues = ldap_get_entries($this->ds, $sri);
			if (@$allValues[0]['cn'][0])
		...
		function mail2fullname($mail)
                {
			$allValues = array();
			$sri = @ldap_search($this->ds, $this->user_context, '(&(mail=' . $mail . ')(|(phpgwaccounttype=u)(phpgwaccounttype=s)))');
			if(!$sri)
				return False;
			$allValues = ldap_get_entries($this->ds, $sri);
			if (@$allValues[0]['cn'][0])
