		...
		function install($times)
		{
			...
			if (($crontab = popen('/bin/sh -c "'.$this->crontab.' -" 2>&1','w')) !== False)
			{
				...
				if ($times !== False)
				{
					...
					$php_version = preg_match("/5./",phpversion()) ? "php" : "php4";
					$cronline .= "cd " . PHPGW_SERVER_ROOT . "/phpgwapi/cron/; $php_version -c /etc/php.ini -q " . PHPGW_SERVER_ROOT . "/phpgwapi/cron/asyncservices.php default\n";
