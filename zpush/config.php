 ...
 */
    define('STATE_DIR', __DIR__.'/states/');
 ...
 */
    define('LOGFILEDIR', __DIR__.'/logs/');
 ...
 */
    ...
    define('BACKEND_PROVIDER', "BackendExpresso");


    //Expresso location
    define('EXPRESSO_PATH', __DIR__.'/..');
...
*/
    ...
    $con = ldap_connect( $ldapConfig['host'] );

    ldap_set_option( $con , LDAP_OPT_PROTOCOL_VERSION , 3 );

    if( isset( $ldapConfig['user'] ) && isset( $ldapConfig['password'] ) )
        ldap_bind( $con , $ldapConfig['user'], $ldapConfig['password'] );

    $GLOBALS['connections']['ldap'] = $con;

    $dbConfig = parse_ini_file(EXPRESSO_PATH . '/prototype/config/PostgreSQL.srv' , true );
    $dbConfig = $dbConfig['config'];

    $rs = '';
    $rs .= ( isset($dbConfig['host']) && $dbConfig['host'] )  ? ' host='.$dbConfig['host'] : '' ;
    $rs .= ( isset($dbConfig['user']) && $dbConfig['user'] )  ? ' user='.$dbConfig['user'] : '' ;
    $rs .= ( isset($dbConfig['password']) && $dbConfig['password'] )  ? ' password='.$dbConfig['password'] : '' ;
    $rs .= ( isset($dbConfig['dbname']) && $dbConfig['dbname'] )  ? ' dbname='.$dbConfig['dbname'] : '' ;
    $rs .= ( isset($dbConfig['port']) && $dbConfig['port'] )  ? ' port='.$dbConfig['port'] : '' ;

    $GLOBALS['connections']['db'] = pg_connect( $rs );


    $rs = pg_query( $GLOBALS['connections']['db'], 'SELECT * FROM phpgw_emailadmin' );
    $row = pg_fetch_assoc( $rs );

    $smtpConfig = parse_ini_file(EXPRESSO_PATH . '/prototype/config/SMTP.srv' , true );
    $GLOBALS['config']['SMTP'] = $smtpConfig['config'];

    // ************************
    //  BackendIMAP settings
    // ************************
    // Defines the server to which we want to connect
    define('IMAP_SERVER', $row['imapserver']);
    // connecting to default port (143)
    define('IMAP_PORT', $row['imapport']);
    // best cross-platform compatibility (see http://php.net/imap_open for options)
    define('IMAP_OPTIONS', (isset($row['imapTLSEncryption']) && $row['imapTLSEncryption'] == 'yes' ) ? '/tls/norsh' : '/notls/norsh');
    ...
    define('IMAP_SENTFOLDER', 'INBOX' . $row['imapdelimiter'] . $row['imapdefaultsentfolder']);
    define('IMAP_TRASHFOLDER', 'INBOX' . $row['imapdelimiter'] . $row['imapdefaulttrashfolder']);
    define('IMAP_DRAFTFOLDER', 'INBOX' . $row['imapdelimiter'] . $row['imapdefaultdraftsfolder']);
    define('IMAP_SPANFOLDER', 'INBOX' . $row['imapdelimiter'] . $row['imapdefaultspamfolder']);
    ...
    define('CALENDAR_SYNC_SIGNED_CALENDARS', false);

    $rs = pg_query( $GLOBALS['connections']['db'], 'SELECT app_id FROM phpgw_applications WHERE app_name = \'zpush\'' );
    $row = pg_fetch_assoc( $rs );


    if(!$row)
       die('Modulo Z-push não Instalado');
