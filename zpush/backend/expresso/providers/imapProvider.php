...
include_once(__DIR__.'/../../../include/mimeDecode.php');
require_once(__DIR__.'/../../../include/z_RFC822.php');


class ExpressoImapProvider extends BackendDiff {
    protected $wasteID;
    protected $sentID;
    protected $server;
    protected $mbox;
    ...
    public function Logon($username, $domain, $password) {
        ...
        $rs = pg_query( $GLOBALS['connections']['db'], 'Select * FROM phpgw_preferences WHERE preference_owner IN (\'-2\',\'-1\',\''.$uidNumber.'\') AND preference_app = \'expressoMail\' ORDER BY preference_owner' );
        ...
        {
            $ra = unserialize($result['preference_value']);
            if(isset($ra['save_in_folder']))
            {
                $this->sentID = $ra['save_in_folder'];
                break;
            }
        }
    ...
    public function GetWasteBasket() {
        ...
        if ($this->wasteID == false) {
            ...
            $wastebaskt = @imap_getmailboxes($this->mbox, $this->server, IMAP_TRASHFOLDER);
...
//        }
        ...
        else if(  $lid == strtolower(IMAP_DRAFTFOLDER) ) {
            $folder->parentid = $this->convertImapId($fhir[0]);
            $folder->displayname = "Drafts";
            $folder->type = SYNC_FOLDER_TYPE_DRAFTS;
        }
        else if($lid == strtolower(IMAP_TRASHFOLDER) ) {
            $folder->parentid = $this->convertImapId($fhir[0]);
            $folder->displayname = "Trash";
            $folder->type = SYNC_FOLDER_TYPE_WASTEBASKET;
            $this->wasteID = $id;
        }
        else if($lid == strtolower(IMAP_SENTFOLDER) ) {
            $folder->parentid = $this->convertImapId($fhir[0]);
            $folder->displayname = "Sent";
            $folder->type = SYNC_FOLDER_TYPE_SENTMAIL;
            //$this->sentID = $id;
    ...
    {
        $ret = imap_deletemailbox($this->mbox , $this->server .$this->getImapIdFromFolderId($id) );
    ...
    protected function getBodyRecursive($message, $subtype, &$body) {
        ...
        if(strcasecmp($message->ctype_primary,"text")==0 && strcasecmp($message->ctype_secondary,$subtype)==0 && isset($message->body)){
            ...
            if(strtolower($message->ctype_parameters['charset']) == 'iso-8859-1')
            {
                $body .=  mb_detect_encoding($message->body) == 'UTF-8' ? $message->body : mb_convert_encoding($message->body , 'ISO-8859-1');
            }
            else
            {
                $body .= $message->body;
