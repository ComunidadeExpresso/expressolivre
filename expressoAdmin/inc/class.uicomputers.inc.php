	...
	*  option) any later version.													   *
	...
	class uicomputers
	{
		...
		function uicomputers()
		{
			...
			$this->bo = CreateObject('expressoAdmin.bocomputers');
			$this->so = $this->bo->so;
			...
			$GLOBALS['phpgw']->js->validate_file('jscode','computers','expressoAdmin');#diretorio, arquivo.js, aplicacao
		}
		...
		function list_computers()
		{
			...
			$GLOBALS['phpgw_info']['flags']['app_header'] = $GLOBALS['phpgw_info']['apps']['expressoAdmin']['title'].' - '.lang('Computers');
			...
			$var = Array(
				...
				'add_action'				=> $GLOBALS['phpgw']->link('/index.php','menuaction=expressoAdmin.uicomputers.add_computer'),
		...
		function add_computer()
		{
			...
			unset($GLOBALS['phpgw_info']['flags']['noheader']);
			unset($GLOBALS['phpgw_info']['flags']['nonavbar']);
			$GLOBALS['phpgw_info']['flags']['app_header'] = $GLOBALS['phpgw_info']['apps']['expressoAdmin']['title'].' - '.lang('Create Computers');
			$GLOBALS['phpgw']->common->phpgw_header();
			...
			$var = Array(
				// LINKS
				'back_url'						=> $GLOBALS['phpgw']->link('/index.php','menuaction=expressoAdmin.uicomputers.list_computers'),
				'form_action'					=> $GLOBALS['phpgw']->link('/index.php','menuaction=expressoAdmin.uicomputers.validade_computers_data_add'),
		...
		function validade_computers_data_add()
		{
			if (($_POST['sambaAcctFlags'] == '[I          ]') && ($_POST['computer_password'] == ''))
			{
				...
				ExecMethod('expressoAdmin.uicomputers.add_computer');
				return;
			}
			...
			ExecMethod('expressoAdmin.bocomputers.create_computer');
		}
		...
		function edit_computer()
		{
			...
			unset($GLOBALS['phpgw_info']['flags']['noheader']);
			unset($GLOBALS['phpgw_info']['flags']['nonavbar']);
			$GLOBALS['phpgw_info']['flags']['app_header'] = $GLOBALS['phpgw_info']['apps']['expressoAdmin']['title'].' - '.lang('Edit Computer');
			$GLOBALS['phpgw']->common->phpgw_header();
			...
			if ($_POST['try_saved'] != 'true')
			{
				...
				$var = Array(
					...
					// LINKS
					'back_url'					=> $GLOBALS['phpgw']->link('/index.php','menuaction=expressoAdmin.uicomputers.list_computers'),
					'form_action'				=> $GLOBALS['phpgw']->link('/index.php','menuaction=expressoAdmin.uicomputers.validade_computers_data_edit')
				);
				$p->set_var($var);
				$p->set_var($this->functions->make_dinamic_lang($p, 'body'));
		...
		function validade_computers_data_edit()
		{
			...
			{
				$_POST['error_messages'] = lang('Computer UID is empty.');
				ExecMethod('expressoAdmin.uicomputers.edit_computer');
				return;
			}

			ExecMethod('expressoAdmin.bocomputers.save_computer');
		}
		...
		function delete_computer()
		{
			...
			unset($GLOBALS['phpgw_info']['flags']['noheader']);
			unset($GLOBALS['phpgw_info']['flags']['nonavbar']);
			$GLOBALS['phpgw_info']['flags']['app_header'] = $GLOBALS['phpgw_info']['apps']['expressoAdmin']['title'].' - '.lang('Delete Computer');
			$GLOBALS['phpgw']->common->phpgw_header();
			...
			$var = Array(
				...
				'computer_description'		=> $computer_data['computer_description'],
				'back_url'					=> $GLOBALS['phpgw']->link('/index.php','menuaction=expressoAdmin.uicomputers.list_computers'),
				'delete_action'				=> $GLOBALS['phpgw']->link('/index.php','menuaction=expressoAdmin.bocomputers.delete_computer'),
			);
			$p->set_var($var);
			$p->set_var($this->functions->make_dinamic_lang($p, 'body'));
		...
		function row_action($action,$type,$uidNumber)
		{
			return '<a href="'.$GLOBALS['phpgw']->link('/index.php',Array(
				'menuaction'		=> 'expressoAdmin.uicomputers.'.$action.'_'.$type,
				'uidnumber'			=> $uidNumber
			)).'"> '.lang($action).' </a>';
		}
