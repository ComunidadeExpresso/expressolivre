<?php
	...
	*  option) any later version.														  *
	...
	class bocomputers
	{
		...
		{
			$this->so = CreateObject('expressoAdmin.socomputers');
		...
		function create_computer()
		{
			...
			if ($_POST['sambaAcctFlags'] == '[I          ]')
			{
				if (!is_file('/home/expressolivre/mkntpwd'))
				{
					...
					ExecMethod('expressoAdmin.uiaccounts.add_computer');
					return false;
				}
			...
			$url = ($GLOBALS['phpgw']->link('/index.php','menuaction=expressoAdmin.uicomputers.list_computers'));
			...
			if (!$this->so->delete_computer_ldap($_POST['computer_dn']))
				{
					...
					ExecMethod('expressoAdmin.uicomputers.list_computers');
					return false;
				}
