...
//                  $options .= '<option value='.$value['gidnumber'].'>'.$value['cn'].'</option>';
	...
	function get_institutional_accounts($params)
	{
		...
		foreach ($this->manager_contexts as $idx=>$context)
		{
			...
			for ($i=0; $i<$entries['count']; ++$i)
			{
				$tr = "<tr class='normal' onMouseOver=this.className='selected' onMouseOut=this.className='normal'><td onClick=edit_institutional_account('".$entries[$i]['uid'][0]."')>" . utf8_decode($entries[$i]['cn'][0]) . "</td><td onClick=edit_institutional_account('".$entries[$i]['uid'][0]."')>" . $entries[$i]['mail'][0] . "</td><td align='center' onClick=delete_institutional_accounts('".$entries[$i]['uid'][0]."')><img HEIGHT='16' WIDTH='16' src=./expressoAdmin/templates/default/images/delete.png></td></tr>";
