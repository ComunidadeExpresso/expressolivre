...
function validate_fields(type)
{
	...
	var handler_validate_fields = function(data)
	{
		if (!data.status) {
			...
			$.ajax({
				type: "POST",
				url: "expressoAdmin/controller.php?action=$this.maillist.create",
				...
				success: function(data, textStatus, jqXHR)
				{
					...
					location.href="./index.php?menuaction=expressoAdmin.uimaillists.list_maillists";
				},
			...
			$.ajax({
				type: "POST",
				url: "expressoAdmin/controller.php?action=$this.maillist.save",
				data: new FormData(document.forms[0]),
				mimeType:"multipart/form-data",
				contentType: false,
				cache: false,
