...
function setOptions( fill, selectId, field )
{
	var select = $('#'+selectId);
	select.html('');
	...
	select.append('<option value="'+value+'">'+fill[value][field]+'</option>');
    }
...
function fillSelect( fill, selectId, field )
{
    ...
    select = setOptions( fill, selectId, field );
...
function entryTag( label, select )
{
	...
	var option = "<option value='-1' disabled='disabled'>"+line+" "+get_lang(label)+" "+line+"</option>";
