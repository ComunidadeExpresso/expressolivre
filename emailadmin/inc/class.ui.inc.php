	...
	* option) any later version.                                                *
	...
	class ui
	{
       ...
       function countProfiles(){
            $return =  $this->boemailadmin->getProfileList();
            (is_array($return) && count($return) > 0) ? $return = count($return) : $return = 0;
    		return $return;
        }

       function addDefaultProfile()
       {
            $emailServer = $_SESSION['phpgw_info']['expressomail']['email_server'];          
            $globalSettings = array(
    			"profileID" => "",
    			"description" => "Expresso",
    			"defaultDomain" => $emailServer['defaultDomain'],
    			"organisationName" => $emailServer['organisationName'],
    			"userDefinedAccounts" => "0"
            );
            ...
            $smtpSettings   = array(
            	...
            	"smtpAuth" => "",
	            "smtpType" => "2", 
	            "smtpLDAPServer" => "127.0.0.1", 
	            "smtpLDAPAdminDN" => $emailServer['smtpLDAPAdminDN'],
	            "smtpLDAPAdminPW" => $emailServer['smtpLDAPAdminPW'],
	            "smtpLDAPBaseDN" =>  $emailServer['smtpLDAPBaseDN]'],
	            "smtpLDAPUseDefault" => "yes"
            );
            ...
            $imapSettings = array(
	            ...
	            'imapDelimiter' => "/",
	            'imapType' => "3",
	            'imapLoginType' => "standard",
	            'imapTLSEncryption' => "1",
	            'imapTLSAuthentication' => "",
	            'imapoldcclient' => "",
	            'imapEnableCyrusAdmin' => "yes",
	            'imapAdminUsername' => "expresso-admin",
	            'imapAdminPW' => $emailServer['imapAdminPW'],
	            'imapEnableSieve' => "yes",
	            'imapSieveServer' => "127.0.0.1",
	            'imapSievePort' => "2000",
	            'imapCreateSpamFolder' => "yes",
	            'imapCyrusUserPostSpam' => "",
	            'imapDefaultTrashFolder' => "Trash",
	            'imapDefaultSentFolder' => "Sent",
	            'imapDefaultDraftsFolder' => "Drafts",
		...
		function editProfile($_profileID='')
		{
			...
			foreach($profileData as $key => $value)
			{
				...
				switch($key)
				{
					...
					case 'imapDelimiter':
						if ($value == '/') 
							$this->t->set_var('selected_'.$key.'_slash','selected="1"');
						elseif ($value == '.')
							$this->t->set_var('selected_'.$key.'_dot','selected="1"');
						break;
					default:
		...
		function listProfiles()
		{
			...
			if ($profileList)
			{
                $profileList_count = count($profileList);
				for ($i=0; $i < $profileList_count; ++$i)
				{
					$linkData = array
					(
		...
		function translate()
		{
			...
			$this->t->set_var('lang_default_folders', lang('Default Folders'));
			$this->t->set_var('lang_trash_folder', lang('Trash Folder'));
			$this->t->set_var('lang_sent_folder', lang('Sent Folder'));
			$this->t->set_var('lang_drafts_folder', lang('Drafts Folder'));
			$this->t->set_var('lang_spam_folder', lang('Spam Folder'));
			$this->t->set_var('lang_spam_settings', lang('spam settings'));
			$this->t->set_var('lang_create_spam_folder', lang('create spam folder'));
			$this->t->set_var('lang_cyrus_user_post_spam', lang('cyrus user post spam'));
			# $this->t->set_var('',lang(''));
