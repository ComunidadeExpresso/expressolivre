...
function smarty_function_wf_calendar($params, &$smarty)
{
	...
	$path = $_SESSION['phpgw_info']['workflow']['server']['webserver_url'] . '/workflow/js/jscalendar/';
...
EOF;
	...
	$output .= $inputFields . '<script type="text/javascript">Calendar.setup({' . $_params_out . '});</script>';
	return $output;
}
?>
