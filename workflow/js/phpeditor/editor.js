...
//			document.body.scrollLeft=cursorpx+44;
	...
	function moz_onKeyPress(evt) {
		...
		if( cancelKey ) {
			...
			if (holdingbackkey)
				do_Backspace();
			if (holdingenterkey) {
				//insertLineAfter(currentLine, new String(getLineContent(currentLine)));
				//updateLine(currentLine-1, '');
				//setCursorPos(0);
				//inputLineEntry.focus();
	...
	function joe_ScrollEnd() {
		...
		currentLine = lastLine;
		redrawInputLine();
		//window.scroll(800,inputLine.style.top);
		//window.scroll(0,0);
		do_End();		
		return false;
	}
	...
	function do_Left(selecting) {
		...
		if( !selecting ) {
			...
			if (!isMoz) {
				content = inputLineEntry.value;
				if (content.charAt(getCursorPos()) == '\r') {
					fixLineGap();
				}
			}
		}
			...
			if (currentLine<lastLine) {
				...
				if (getCursorPos()==_editline.length) {
					if (!isMoz) {
						...
						inputLineEntry.value = inputLineEntry.value.substring(0,inputLineEntry.selectionStart) + 
											   inputLineEntry.value.substring(inputLineEntry.selectionEnd,inputLineEntry.value.length);
					}
					checkDirty();
					if (offsetLine == 0)
						glueCurrentLine(1);
	...
	function do_End() {
		...
		removeAddedSpaces();
		setCursorPos(inputLineEntry.value.length);
		if (getCursorPos()*letterWidth + linenumbers.style.width > window.parent.innerWidth) {
	...
	function fixmousepaste()
	{
		fixPastedReturns(true);
		checkPastedReturns = false;
	}
	...
	function isNormalKey(charCode)
	{
		return ( ( parseInt( charCode ) >= 48 ) && ( parseInt( charCode ) <= 90 ) )  || 
