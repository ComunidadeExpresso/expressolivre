...
document.getElementById(this._items[i]).style.display="none";
}
}
this._isHide=true;}
}
...
document.getElementById(this._items[i]).style.display="";
}
}
}
}
...
this._eClick="onPopItemMOut(event,this,'',_mmObj);"+clickParam; break;case 'address':
this._eClick="_openURL('"+clickParam+"')";break;case 'a':
this._eClick="onPopItemMOut(event,this,'',_mmObj);"+"_openURL('"+clickParam+"')";break;default:
this._eClick='';}
this._type='';
...
{
var result=this.BuildStyle();
var level=-1;this.SetParent(this);result+=this.Draw(level++);level++;for(var i=0;i<=this._itemIndex;i++)
{
...
{
this._items[i]._icon='<td nowrap style="cursor:pointer;padding-left:'+iIcon+'px;">';}
else
{
switch(this._items[i]._tmpIcon)
{
case "":
this._items[i]._icon='<td nowrap style="padding-left:24px;">';break;case '0':
this._items[i]._icon='<td nowrap>';break;default:
this._items[i]._icon='<td style="cursor:pointer;cursor:hand" nowrap class="TIcon'+this._items[i]._parent._index+'"><img class="TIcon'+this._items[i]._parent._index+'" src="'+this._items[i]._tmpIcon+'" /></td><td style="cursor:pointer;cursor:hand" nowrap>';}
}
...
result+='<div style="position:absolute;display:none;filter: blur( direction=135, strength='+this._shadow._distance+', add=1);" id="sh_'+this._id+'" align="left"><div></div></div>';else
result+='<div style="position:absolute;display:none;" id="sh_'+this._id+'"></div>';}
if((this._type=='free') && (this._position=='absolute'))
{
result+='<table class="TMenu'+this._index+'" id="fr_'+this._id+'"';result+=' cellspacing="'+this._cellSpacing+'" style="cursor:pointer;position:absolute;display:none;">';result+='<tr><td nowrap class="TMMHeader'+this._index+'"';result+=' onclick="onMMHeaderClick(event,'+this._name+')"';result+='>'+this._headerText+'</td></tr>';result+='<tr><td nowrap> </td></tr>';result+='</table>';}
...
if((this._type=='free') && (this._position=='absolute'))
{
var colSpan=this._itemIndex+1;result+='<tr><td nowrap  colspan="'+colSpan+'" class="TMMHeader'+this._index+'"';result+=' onclick="onMMHeaderClick(event,'+this._name+')">'+this._headerText+'</td></tr>';}
}
result+='<tr>';}
else
{
if((this._type=='free') && (this._position=='absolute'))
{
result+='<td nowrap  class="TMMHeader'+this._index+'"';result+=' onclick="onMMHeaderClick(event,'+this._name+')">'+this._headerText+'</td>';}
}
if(this._itemIndex >-1)
{
...
{
result1+=' onclick="onMainMOver(event,this,\''+this._items[i]._id+'\','+level+','+this._name+',\''+escape(this._items[i]._status)+'\',true)" onmouseover="onMainMOver(event,this,\''+this._items[i]._id+'\','+level+','+this._name+',\''+escape(this._items[i]._status)+'\')"';if(this._popOnClick)
...
{
//expandHTML='<td nowrap  class="TMMExpand'+this._items[i]._parent._index+'">'+this._items[i]._parent._expandIcon._symbol+'</td>';isExpandIcon=true;
}
}
else
{
result1+=' onmouseover="onMainMOver(event,this,\'\','+level+','+this._name+',\''+escape(this._items[i]._status)+'\')"';result1+=' onmouseout="onMainMOut(event,this,\'\','+this._name+')"';result1+=' onclick="onMainMOut(event,this,\'\','+this._name+');'+this._items[i]._eClick+'"';}
var iIcon=parseInt(this._items[i]._tmpIcon);if((iIcon > 0))
{
this._items[i]._icon='<td nowrap  style="padding-left:'+iIcon+'px;">';}
else
{
switch(this._items[i]._tmpIcon)
{
...
this._items[i]._icon='<td nowrap  style="padding-left:30px;padding-right:16px">';else
this._items[i]._icon='<td nowrap  style="padding-left:30px">';break;case '0':
if(isExpandIcon && expandHTML=='')
this._items[i]._icon='<td nowrap   style="padding-right:16px">';else
this._items[i]._icon='<td nowrap >';break;default:
if(isExpandIcon && expandHTML=='')
this._items[i]._icon='<td nowrap  class="TIcon'+this._items[i]._parent._index+'"><img class="TIcon'+this._items[i]._parent._index+'" src="'+this._items[i]._tmpIcon+'"  /></td><td nowrap   style="padding-right:16px">';else
this._items[i]._icon='<td nowrap  class="TIcon'+this._items[i]._parent._index+'"><img class="TIcon'+this._items[i]._parent._index+'" src="'+this._items[i]._tmpIcon+'"  /></td><td nowrap >';}
}
...
{
var result1='';result1+='';if(this._direction=='horizontal')
result+=result1;else
result+='<tr>'+result1+'</tr>';}
...
{
if(_browser._name=='IE')
result+='<div style="position:absolute;visibility:hidden;filter: blur( direction=135, strength='+this._parent._pop._shadow._distance+', add=1);" id="sh_'+this._id+'" align="left"><div  onmouseout="hideAll('+this._parent._name+')"></div></div>';else
result+='<div style="position:absolute;visibility:hidden;" id="sh_'+this._id+'" align="left"></div>';}
if(is_ie)
{
	var lineHeight = this._items.length*26;
	var lineWidth = this._itemWidth * 1.074;
	if(lineHeight > 200)
		lineHeight = 200;
	sizeMenu="display:none;height:"+lineHeight+"px;overflow-x:hidden;overflow-y:auto;width:" + lineWidth + "px;";
}
else
	sizeMenu="display:none;max-height:50%;overflow:auto;width:" + this._itemWidth +"px;";

result+='<div class="TPopUp'+this._parent._index+'" id="'+this._id+'" style='+sizeMenu+'"background:#FFFFFF;filter:alpha(opacity='+this._parent._pop._alpha+');">';for(var i=0;i<=this._itemIndex;i++)
{
if(this._items[i]._label !='-')
{
if(this._items[i]._itemIndex >-1)
{
result+='<div style="border-right:1px solid #FFFFFF;width:91%" class="TPopUpItem'+this._items[i]._parent._index+'" id="di_'+this._items[i]._id+'">';
result+='<table border=0 class="TPopUpItem'+this._items[i]._parent._index+'" cellspacing="0" cellpadding="2"';
...
if(this._items[i]._parent._pop._expandIcon._create)
	result+='<td nowrap  class="TExpand'+this._items[i]._parent._index+'">'+this._items[i]._parent._pop._expandIcon._symbol;

result+='</tr></table>';result+='</div>';
}
else
{
...
{
result+='<div style="border-right:1px solid #FFFFFF;width:91%" class="TPopUpItem'+this._items[i]._parent._index+'">';
result+='<table border=0 class="TPopUpItem'+this._items[i]._parent._index+'" cellspacing="0" cellpadding="0" ';
...
result+=' onclick="hideAll('+this._items[i]._parent._name+');'+this._items[i]._eClick+'"';result+='><tr>'+this._items[i]._icon+this._items[i]._label+'</td>';
//result+='<td nowrap  class="TExpand'+this._items[i]._parent._index+'">';
result+='</tr></table>';result+='</div>';}
}
}
else
...
function BuildStyle()
{
var result='';var tyH;var tyW;result+='<style type="text/css">';result+='table.TMenu'+this._index+'{';result+='cursor:pointer;cursor:hand';result+=';visibility:hidden';result+=';position:'+this._position;tyW=typeof(this._width);if(tyW=='string')
{
this._width=this._width.toLowerCase();if(this._width !='auto' || this._width !='')
result+=';width:'+this._width+'px';}
else
result+=';width:'+this._width+'px';
result+=';border-top:'+this._border._top;result+=';border-right:'+this._border._right;result+=';border-bottom:'+this._border._bottom;result+=';border-left:'+this._border._left;result+=';background-color:'+this._background._color;result+=';background-image:'+this._background._image;result+=';background-position:'+this._background._position;result+=';background-repeat:'+this._background._repeat;result+=';}';result+='table.TMenuItemChild'+this._index+'{';result+='width:100%';result+=';font-family:'+this._font._family;result+=';font-size:'+this._font._size;result+=';cursor:pointer;font-weight:'+this._itemText._weight;result+=';text-align:'+this._itemText._align;result+=';color:'+this._itemText._color;result+=';text-decoration:'+this._itemText._decoration;result+=';white-space:'+this._itemText._whiteSpace;result+=';}';
...
{
_arRegisterTriggerPopIndex[mmObj._index]++;var j=_arRegisterTriggerPopIndex[mmObj._index];_arRegisterTriggerPopID[mmObj._index][j]=elmID;
if(document.all)
var el=document.all(elmID);
else if(document.getElementById)
var el=document.getElementById(elmID);
if(el.className && el.className.indexOf("TMenuItem") !=-1)
{
...
if(el && el.className.indexOf("TMenuItem") !=-1)
{
...
if(elTable)
{
	...
	elTable.style.textAlign=mmObj._pop._itemText._align;elTable.style.textDecoration=mmObj._pop._itemText._decoration;
	elTable.style.whiteSpace=mmObj._pop._itemText._whiteSpace;elTable.style.fontWeight=mmObj._pop._itemText._weight;
}
}
_arRegisterTriggerPopID[mmObj._index][i]=null;}
...
{var elm=document.getElementById(elmID);var parentElm=document.getElementById(parentElmID);}else if(document.getElementById)
{
...
if(document.all && document.all(ID).style)
...
if(mmObj._pop._shadow._create){
	if (document.getElementById('sh_'+ID))
		document.getElementById('sh_'+ID).style.visibility="hidden";
}
}
}
...
function onMainMOver(event,elm,popID,level,mmObj,status,onclicked)
{
hideAllPopUpMenu(_mmObj);removeAllTriggerPopID(_mmObj);window.status=unescape(status);clearTriggerHideAll(mmObj);
...
if(_arMMClick[mmObj._index])
{
hideAllPopUpMenu(mmObj);removeAllTriggerPopID(mmObj);
if(popID !='')
{
_mmObj = mmObj;
if(onclicked){
	...
	showPopUpMenu(elm.id,popID,'','mm');
}
else
	_aId = window.setTimeout("showPopUpMenu('"+elm.id+"','"+popID+"','','mm');",500);
...
window.releaseEvents(event.MOUSEMOVE);elm.style.top=frElm.style.top;elm.style.left=frElm.style.left;mmObj._left=xPos-_mmHeaderMoveObjCorrectionX;mmObj._top=yPos-_mmHeaderMoveObjCorrectionY;frElm.style.visibility='hidden';elm.style.visibility='visible';frElm.style.display='none';elm.style.display='';}
else
{
_mmHeaderMoveObjCorrectionX=xPos-mmObj._left;_mmHeaderMoveObjCorrectionY=yPos-mmObj._top;elm.style.visibility='hidden';frElm.style.visibility='visible';elm.style.display='none';frElm.style.display='';mmObj._headerClickState=true;_mmHeaderMoveObj=mmObj;document.onmousemove=onMMHeaderMove;if(_browser._name=='Netscape' && _browser._version==4)
